...
class idOS:
    ...
    def welcome(self):
        print(
            f"""
        {Fore.GREEN + Style.BRIGHT}idOS {Fore.BLUE + Style.BRIGHT}Auto BOT
            """
            f"""
    ...
    def decode_token(self, token: str):
        try:
            ...
            self.log(
                ...
                f"{Fore.RED + Style.BRIGHT} Fetch User Id Failed {Style.RESET_ALL}"
    ...
    async def auth_verify(self, account: str, address: str, auth_message: dict, proxy_url=None, retries=5):
        url = f"{self.BASE_API}/auth/verify"
        data = json.dumps(self.generate_payload(account, address, auth_message))
        headers = {
            **self.HEADERS[address],
            "Content-Length": str(len(data)),
            "Content-Type": "application/json",
    ...
    async def user_points(self, address: str, proxy_url=None, retries=5):
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                self.log(
                    ...
                    f"{Fore.RED+Style.BRIGHT} Fetch idOS Points Failed {Style.RESET_ALL}"
                    f"{Fore.MAGENTA+Style.BRIGHT}-{Style.RESET_ALL}"
                    f"{Fore.YELLOW+Style.BRIGHT} {str(e)} {Style.RESET_ALL}"
                )
    ...
    async def daily_check(self, address: str, proxy_url=None, retries=5):
        url = f"{self.BASE_API}/user-quests/complete"
        data = json.dumps({"questName": "daily_check", "userId": self.user_ids[address]})
        headers = {
            **self.HEADERS[address],
            "Authorization": f"Bearer {self.access_tokens[address]}",
            "Content-Length": str(len(data)),
    ...
    async def process_user_login(self, account: str, address: str, use_proxy: bool, rotate_proxy: bool):
        ...
        if is_valid:
            ...
            auth_message = await self.auth_message(address, proxy)
            if not auth_message: return

            auth_verify = await self.auth_verify(account, address, auth_message, proxy)
            if not auth_verify: return False

            self.access_tokens[address] = auth_verify.get("accessToken")
            self.refresh_tokens[address] = auth_verify.get("refreshToken")

            user_id = self.decode_token(self.access_tokens[address])
            if not user_id: return False

            self.user_ids[address] = user_id
    ...
    async def process_accounts(self, account: str, address: str, use_proxy: bool, rotate_proxy: bool):
        ...
        if logined:
            ...
            checkin = await self.daily_check(address, proxy)
            if checkin:
                self.log(
                    f"{Fore.CYAN+Style.BRIGHT}Check-In:{Style.RESET_ALL}"
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"{Fore.CYAN + Style.BRIGHT}[ {datetime.now().astimezone(wib).strftime('%x %X %Z')} ]{Style.RESET_ALL}"
            f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
            f"{Fore.RED + Style.BRIGHT}[ EXIT ] idOS - BOT{Style.RESET_ALL}                                       "                              
        )